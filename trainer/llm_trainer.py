...
class LLMPPO(OnPolicyAlgorithm):
    """
    ...
    def __init__(
        ...
        self.policy = LLMAgent(action_dim=env.action_space.n, input_dim=self.observation_space.shape,
    ...
    def _setup_model(self) -> None:
        super(LLMPPO, self)._setup_model()
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in generator:
                ...
                values, log_prob, entropy, recon_loss, embed_loss = self.policy.evaluate_actions(observations, actions)
                ...
                # ac_loss + rep_loss
                loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss + recon_loss + embed_loss
        ...
        # Logs
        self.logger.record("train/reconstruct_loss", np.mean(recon_losses))
        self.logger.record("train/embedding_loss", np.mean(embed_losses))
    ...
    def collect_rollouts(self, env: VecEnv, callback: BaseCallback, rollout_buffer: RolloutBuffer,
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                observations_ = observations.unsqueeze(1)
                if first:
                    tensor_obs_rollout = observations_
                    first = False
                else:
                    tensor_obs_rollout = th.cat([tensor_obs_rollout, observations_], 1)
                    if tensor_obs_rollout.shape[1] > 30:
                        tensor_obs_rollout = tensor_obs_rollout[:, -30:, :]

                action, value, log_prob = self.policy(tensor_obs_rollout)
            ...
            rollout_buffer.add(add_obs, action, rewards, self._last_episode_starts, value, log_prob=log_prob)
